...
int db_music_search(music_t* list, const int size, const char* search){
	if(get_db_state() == DB_OPEN){
		char* sql = db_sql_generator("SELECT * FROM %s WHERE %s LIKE ?;", MUSIC, MUSIC_PATH);
		sqlite3_stmt* stmt = NULL;
		sqlite3_prepare_v2(get_db(), sql, -1, &stmt, 0);
		sqlite3_bind_text(stmt, 1, (char*)search, -1, SQLITE_TRANSIENT);
		int index = 0;
		while(sqlite3_step(stmt) == SQLITE_ROW && index < size){
			music_t music;
